...
class InscripcionController extends CI_Controller {
	...
	public function innerInscripcionOptionsComponet($inscripcion,$is_anulated){
		$optionsToContainer=[
			...
			($is_anulated?"<button class='btn btn-success' onclick='ins.cancel(".$inscripcion["id_inscripcion"].");'><i class='fa fa-fw fa-calendar-check-o' aria-hidden='true'></i> Entrevista</button>":"")
		];
		$detailsOption=($is_anulated?'<div onclick="modalDataInscrito.loadData('.$inscripcion["id_inscripcion"].');"><i class="fa fa-eye"></i></div>':'');
