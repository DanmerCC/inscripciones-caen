<page backcolor="#FEFEFE" style="font-size: 12pt; margin 10%; border:1" margin="1">
	<bookmark title="Solicitud de Inscripcion" level="0" ></bookmark>
	<div style="height: 100" border="1" height="100">
		
		<table style="width:100%;height: 100%;" border="" >
			
			<tr style="width:100%;height: 5%;">
				
				<td style="width:66%;">
				</td>
				<td style="width:34%" rowspan="3" border="">
				<table>
					<tr>
						<td style="width:10%"></td>
						<td></td>
						<td style="width:10%"></td>
					</tr>
					<tr>
						<td style="width:10%"></td>
						<td border="1" style="width:80%">SOLICITO:<br>
					Admision para el programa :<br><u><span><?=$programa?></span></u></td>
						<td style="width:10%"></td>
					</tr>
					<tr>
					<td style="width:10%"></td>
						<td></td>
						<td style="width:10%"></td>
					</tr>
				</table>
					
				</td>
				<td rowspan="17" style="background-color: #696969;width:3%;">
				</td>
			</tr>
			<tr style="height: 200px;" border="1">
				
				<td style="width:66%;" >
				
					<img  src="assets/img/logo-caen.jpg" alt="Logo">
				</td>
				<td style="">
					
				</td>
			</tr>
			<tr style="height: 5%;" border="1">
				<td style="width:66%;height: 10px" >
					
				</td>
				<td style="">

				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					<h2>Señor Director General</h2>
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr><tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
				<p>del Centro de Altos Estudios Nacionales – Escuela de Posgrado</p>
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">

				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					<p>
					Yo, <font face="times "><i ><u><?=$nombres_completo;?></u></i></font>, identificado con DNI Nº <font face="times "><i><u><?=$documento;?></u></i></font> de profesión <font face="times "><i><u><?=$profesion;?></u></i></font>, con domicilio legal en <font face="times "><i><u><?=$domicilio?></u></i></font>
				en el distrito de <font face="times "><i><u><?=$domicilio_distrito?></u></i></font>, teléfono <font face="times "><i><u><?=$telefono?></u></i></font> y correo electrónico <font face="times "><i><u><?=$email?></u></i></font>, me presento ante ud. y expongo lo siguiente:
					</p>
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;" >

				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					<p>
						Que deseando participar en el programa de <font face="times "><i><u><?=$programa?></u></i></font>, a dictarse a partir del <font face="times "><i><u><?=$fechatexto_start_program?></u></i></font>, solicito a Ud. señor Director General del CAEN - EPG, tenga a bien disponer mi inscripción como postulante en el proceso de admisión correspondiente a dicho programa.
					</p>
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td></td>
			</tr>
			<tr style="height: 5%;">

				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
				<p>
					Al respecto, dejo constancia que el suscrito ha tomado conocimiento de los requisitos necesarios para postular al citado programa, de acuerdo a las disposiciones vigentes del Centro de Altos Estudios Nacionales y de la Ley Universitaria.
				</p>
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">

				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					<p>
						Por lo expuesto, Señor Director, solicito a Ud. acceder a mi pedido
					</p>
				</td>
				<td></td>
			</tr>
			
			<tr style="">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td style="width:5%;"></td>
				
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
				<td rowspan="16" style="background-color: #ff6347;width:3%;">
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:45%;height: 10px">
				<p>
						Chorrillos, <font face="times "><i><u><?=$fecha_actual;?></u></i></font>
				</p>
				</td>

				<td></td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:45%;height: 10px" colspan="2">
					
				</td>
				<td style="width:45%;height: 10px" align="rigth">
					<p>
						------------------------------------
					</p>
					<p>
							Firma del interesado
					</p>
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td style="width:5%"></td>
				<td style="width:90%;height: 10px" colspan="2">
					
				</td>
				<td>
				</td>
			</tr>
			<tr style="height: 5%;">
				<td colspan="2">
				<p style="text-align:left" >
				
				Nota: No se aceptarán solicitudes diferentes a este formato.
				</p>
				</td>
			</tr>
		</table>
	</div>
<page>

